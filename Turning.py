...
def turn_drivetrain_right():
    # Activate motor driver
    ENB.value(1)  # Enable Motor B
    ENA.value(1)  # Enable Motor A
    while encoder_position_a != -encoder_position_b:
        # Move forward
            IN1.value(1)  # Motor A forward
            IN2.value(0)
            IN3.value(1)  # Motor B forward (Motor B is backwards compared to Motor A)
            IN4.value(0)
    ENB.value(1)
    ENA.value(1)
    IN1.value(1)
    ...
    IN3.value(1)
    IN4.value(1)
def turn_drivetrain_left():
    # Activate motor driver
    ENB.value(1)  # Enable Motor B
    ENA.value(1)  # Enable Motor A
    while -encoder_position_a != encoder_position_b:
        # Move forward
            IN1.value(0)  # Motor A forward
            IN2.value(1)
